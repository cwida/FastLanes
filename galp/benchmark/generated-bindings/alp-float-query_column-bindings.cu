...
template <>
bool query_column<float, flsgpu::device::ALPColumn<float>>(const flsgpu::device::ALPColumn<float> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 32 && unpacker == enums::Unpacker::OldFls &&
		...
		return kernels::host::query_column<
		    ...
		    flsgpu::device::ALPColumn<float>>(column, magic_value, n_samples);
	}
	if (unpack_n_vectors == 1 && unpack_n_values == 32 && unpacker == enums::Unpacker::OldFls &&
