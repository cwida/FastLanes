#ifndef FLS_PRIMITIVE_ALP_DECODE_HPP
#define FLS_PRIMITIVE_ALP_DECODE_HPP

// NOLINTBEGIN

#include "fls/primitive/alp/encode.hpp"
#include <cstdint>
...
void magic_convert_decode(const int64_t* digits, uint8_t fac_idx, uint8_t exp_idx, double* out_p) {
	int64_t factor = u_fact_arr[fac_idx];
	double  frac10 = frac_arr[exp_idx];

	for (size_t i {0}; i < 1024; ++i) {
		auto digit = digits[i];

		out_p[i] = int64_to_double(digit * factor) * frac10;
	}
}
...
inline void decode(const uint64_t* digits, uint8_t fac_idx, uint8_t exp_idx, double* out_p) {
	uint64_t factor = u_fact_arr[fac_idx];
	double   frac10 = frac_arr[exp_idx];

	for (size_t i {0}; i < 1024; ++i) {
		auto digit = digits[i];

		out_p[i] = static_cast<double>(static_cast<int64_t>(digit * factor)) * frac10;
	}
}
...
#endif // FLS_PRIMITIVE_ALP_DECODE_HPP
