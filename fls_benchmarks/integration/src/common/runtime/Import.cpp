...
void parseColumns(runtime::Relation& r, std::vector<ColumnConfigOwning>& cols, std::string dir, std::string fileName) {
	...
	for (auto& col : colsC) {
		if (!std::ifstream(cachedir + fileName + "_" + col.name)) { allColumnsMMaped = false; }
	}
	...
	if (!allColumnsMMaped) {
		...
		if (!relationFile.is_open()) throw runtime_error("csv file not found: " + dir);
		string   line;
...
void importFLS(std::string dir, Database& db) {
	{
		...
		rel.name     = CFG::NAME;
		auto columns = configX({{"first", make_unique<algebra::Integer>()}});
