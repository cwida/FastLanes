...
struct PerfEvents {
	...
	void
	timeAndProfile(std::string s, uint64_t count, std::function<void()> fn, uint64_t repetitions = 1, bool mem = false);
};
...
void PerfEvents::timeAndProfile(
    std::string s, uint64_t count, std::function<void()> fn, uint64_t repetitions, bool mem) {
	using namespace std;
	// warmup round
	...
	for (; performedRep < repetitions || gettime() - start < 0.5; ++performedRep) {
		fn();
	}
	double end = gettime();
	readAll();
	...
	std::cout << setw(20) << s << "," << setw(printFieldWidth) << (runtime * 1e3 / performedRep) << ",";
#ifdef __linux__
