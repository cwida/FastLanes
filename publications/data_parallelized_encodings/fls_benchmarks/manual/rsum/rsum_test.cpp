...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// publications/data_parallelized_encodings/fls_benchmarks/manual/rsum/rsum_test.cpp
// ────────────────────────────────────────────────────────
...
TEST_F(rsum, test_scalar_delta_lemire_d1_8) {
	manual::delta::lemire_d1_8(helper::delta_08, output08, helper::bases_08);
	for (int i = 0; i < 1024; ++i) {
		ASSERT_EQ(output08[i], helper::array_08[i]);
	}
}

TEST_F(rsum, test_scalar_delta_lemire_d1_16) {
	manual::delta::lemire_d1_16(helper::delta_16, output16, helper::bases_16);
	for (int i = 0; i < 1024; ++i) {
		ASSERT_EQ(output16[i], helper::array_16[i]);
	}
}

TEST_F(rsum, test_scalar_delta_lemire_d1_32) {
	manual::delta::lemire_d1_32(helper::delta_32, output32, helper::bases_32);
	for (int i = 0; i < 1024; ++i) {
		ASSERT_EQ(output32[i], helper::array_32[i]);
	}
}

TEST_F(rsum, test_scalar_delta_lemire_d1_64) {
	manual::delta::lemire_d1_64(helper::delta_64, output64, helper::bases_64);
	for (int i = 0; i < 1024; ++i) {
		ASSERT_EQ(output64[i], helper::array_64[i]);
	}
}
...
// this implementation fails does not pass the test as the instruction has different behaviour.
...
// this implementation does not exist at first place.
...
TEST_F(rsum, test_scalar_avx512_d1_64) {
	manual::delta::avx512_d1_64(helper::delta_64, output64, helper::bases_64);
	for (int i = 0; i < 1024; ++i) {
		ASSERT_EQ(output64[i], helper::array_64[i]);
	}
}
