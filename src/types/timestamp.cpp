...
int64_t parse_timestamp(std::string_view ts) {
    ...
    if (ts.size() == 22 &&
        ts[2] == '/' && ts[5] == '/' && ts[10] == ' ' &&
        ts[13] == ':' && ts[16] == ':' && ts[19] == ' ' &&
        ((ts[20] == 'A' && ts[21] == 'M') || (ts[20] == 'P' && ts[21] == 'M')))
    {
        ...
        if (!(1 <= mon && mon <= 12) || !(1 <= day && day <= 31) ||
            !(0 <= hour && hour <= 12) || !(0 <= minute && minute <= 59) ||
            !(0 <= second && second <= 59))
        {
        ...
        auto epoch = std::chrono::sys_days{
            std::chrono::year{kEpochYear} / std::chrono::month{kEpochMonth} / std::chrono::day{kEpochDay}
        };
        std::chrono::year_month_day ymd{
            std::chrono::year{year} / std::chrono::month{static_cast<unsigned>(mon)} / std::chrono::day{static_cast<unsigned>(day)}
        };
        ...
        ts[8] == ' ' && ts[11] == ':' && ts[14] == ':')
    {
        ...
        if (idx < ts.size() && ts[idx] == '.') {
            ...
            while (end < ts.size() &&
                   std::isdigit(static_cast<unsigned char>(ts[end])) &&
                   (end - start) < 6)
            {
                ++end;
            }
        ...
        if (idx < ts.size() && (ts[idx] == '+' || ts[idx] == '-')) {
            sign = (ts[idx++] == '+') ? +1 : -1;
            if (idx + 1 >= ts.size() ||
                !std::isdigit(static_cast<unsigned char>(ts[idx])) ||
        ...
        // chrono
        auto epoch = std::chrono::sys_days{
            std::chrono::year{kEpochYear} /
            std::chrono::month{kEpochMonth} /
        ...
        auto tp = std::chrono::sys_days{ymd}
                ...
                + std::chrono::seconds{second}
                + frac
                - std::chrono::hours{sign * off_h}
                - std::chrono::minutes{sign * off_m};

        return std::chrono::duration_cast<std::chrono::microseconds>(tp - epoch).count();
    }

    // ── branch C: “YYYY-MM-DD[T| ]hh:mm:ss[.ffffff][±HH[:MM]]” ────
