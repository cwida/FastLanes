...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// benchmark/include/cycle_counter.hpp
// ────────────────────────────────────────────────────────
...
inline BENCHMARK_ALWAYS_INLINE int64_t Now() {
	...
	//  return virtual_timer_value;
#if defined(__APPLE__)
	return static_cast<int64_t>(get_counters());
#else
	return cycles.now();
#endif
...
static std::vector<CPUInfo::CacheInfo> GetCacheSizesFromKVFS() {
	...
	while (true) {
		...
		f >> info.size;
		if (f.fail()) {
			PrintErrorAndDie("Failed while reading file '", f_path, "size'");
		}
		if (f.good()) {
			f >> suffix;
			if (f.bad()) {
...
inline std::vector<CPUInfo::CacheInfo> GetCacheSizesMacOSX() {
	std::vector<CPUInfo::CacheInfo> res;
...
inline FLS_BENCH_MAYBE_UNUSED std::string GetSystemName() {
...
public:
	...
	void Run(benchmark::BenchmarkReporter::Run run) {
		if (m_enable_print) {
			benchmark::CSVReporter::PrintRun(run);
		}
