...
#include <filesystem> // std::filesystem::exists, create_directories, remove_all
...
class FastLanesReaderTester : public ::testing::Test {
	...
	void SetUp() override {
		...
		if (!std::filesystem::exists(fls_dir_path_process_specific)) {
			std::filesystem::create_directories(fls_dir_path_process_specific);
		}
	};

	void TearDown() override {
		if (std::filesystem::exists(fls_dir_path_process_specific)) {
			std::filesystem::remove_all(fls_dir_path_process_specific);
		}
	}
...
public:
	...
	void TestConstantness(const vector<n_t>& constant_indexes) const {
		...
		const auto* cols = footer.m_column_descriptors();
		const auto  ncol = cols ? cols->size() : 0;

		for (const auto col_idx : constant_indexes) {
			const auto fb_idx = static_cast<flatbuffers::uoffset_t>(col_idx);
			ASSERT_LT(fb_idx, ncol) << "column index OOB";
			const auto* col_descriptor = cols->Get(fb_idx);
	...
	void TestEquality(const string_view table) const {
		...
		for (flatbuffers::uoffset_t col_idx = 1; col_idx < ncol; ++col_idx) {
			const auto* col_descriptor = cols->Get(col_idx);
			ASSERT_EQ(col_descriptor->total_size(), 0) << col_idx << " should be of size 0";
		}
	}
	...
	void TestEquality(const vector<n_t>& equal_cols) const {
		...
		for (const auto col_index : equal_cols) {
			const auto fb_idx = static_cast<flatbuffers::uoffset_t>(col_index);
			ASSERT_LT(fb_idx, ncol) << "column index OOB";
			const auto* col_descriptor = cols->Get(fb_idx);
			ASSERT_EQ(col_descriptor->total_size(), 0)
			    << "size of column " << col_index << " should be 0 (equal to another column)";
		}
	}
	...
	void TestMap1To1(const vector<n_t>& target_column_indexes) const {
		...
		for (const auto col_index : target_column_indexes) {
			...
			const auto* rpn = col_descriptor->encoding_rpn();
			ASSERT_NE(rpn, nullptr) << "missing encoding_rpn for col " << col_index;
			const auto* toks = rpn->operator_tokens();
			ASSERT_NE(toks, nullptr) << "missing operator_tokens for col " << col_index;
			ASSERT_GT(toks->size(), 0u) << "no operator tokens for col " << col_index;

			ASSERT_TRUE(is_1_to_1(toks->Get(0))) << "  " << col_index;
		}
	}
